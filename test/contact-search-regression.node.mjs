...
async function searchCase(body, { authorized = true, fail = false } = {}) {
  let handler;
  const calls = [];
  const context = vm.createContext({
    URL, Request, Response, console: { error() {} },
    Deno: { env: { get: () => '' }, serve: fn => { handler = fn; } },
    createDbClient: () => ({ rpc: async (name, args) => {
      calls.push({ name, args });
      if (name === 'verify_session') return { data: { valid: authorized, agent: authorized ? { id: 'fixture-agent', role: 'agent' } : null }, error: null };
    ...
    fetch: () => { throw new Error('Search must never contact a dialer'); },
  });
