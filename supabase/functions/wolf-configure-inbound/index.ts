...
HUMAN FIRST POLICY: Before transferring any caller, ask for their full name. Repeat the name back and wait for confirmation that you heard it correctly. A caller's stated name alone does not establish a verified case match; it confirms only who is on the line. When trusted lead context identifies an intended person for this phone number, compare the caller's stated name against it but do not reveal the intended person's name or case details to an unverified caller. No shortcut bypasses name confirmation: a direct request for the agent, agreement to hold, or eagerness to connect does not substitute for confirming the caller's name.
...
2. RETURNING A CALL: "Thank you for returning our call. May I have your full name?" After they respond, repeat the name back: "I have [name]. Is that correct?" Once confirmed: "${agentName} would like to speak with you directly. Please hold while I connect you." Then invoke transfer immediately.
3. WHY THIS NUMBER / WHAT IS THIS ABOUT: "I help connect callers with ${agentName}. I do not have the details to discuss, but ${agentName} can explain. May I have your full name so I can connect you?"
4. UNKNOWN AGENT: "That is okay. You do not need to know ${agentName} personally. May I have your full name first, and then I can connect you so they can clarify?"
5. CASE QUESTIONS: "I cannot confirm case details. ${agentName} can help with your question. May I have your name so I can connect you?" Do not assume a case exists or invent urgency, authority, deadlines, balances, or private information.
6. When the caller agrees or directly asks for the agent and their name is confirmed, say "Please hold while I connect you to ${agentName}." Immediately invoke transfer using the configured destination. Do not ask for permission twice. Remain silent while the transfer connects and allow the agent\'s voicemail greeting and recording to complete if the agent does not answer. Do not disconnect a transfer because you hear the agent\'s voicemail.
...
Deno.serve(async (req: Request) => {
  ...
  const schedulerSig = req.headers.get("x-scheduler-signature") || "";
  let authOk = serviceRoleKey && authorization === `Bearer ${serviceRoleKey}`;
  if (!authOk && schedulerSig) {
    try {
      const adminClient = createClient(supabaseUrl, serviceRoleKey, { auth: { persistSession: false } });
      const { data: configRow } = await adminClient.from("system_config").select("value").eq("key", "dialer_scheduler_secret").maybeSingle();
      const secret = configRow?.value || "";
      if (secret.length >= 32) {
        ...
        const sigBytes = await crypto.subtle.sign("HMAC", key, new TextEncoder().encode("configure_inbound"));
        const expected = Array.from(new Uint8Array(sigBytes)).map(b => b.toString(16).padStart(2, "0")).join("");
        authOk = schedulerSig === expected;
      }
    } catch { /* HMAC auth failed */ }
  }
  if (!authOk) {
