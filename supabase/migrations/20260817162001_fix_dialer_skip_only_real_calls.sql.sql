-- Fix dialer_next_batch: only skip leads that have a call with a real provider_call_id
-- (i.e., actually sent to Bland.ai). Leads with only stale/empty provider_call_id calls
-- should be re-dialable.
...
-- Only skip leads that were ACTUALLY sent to Bland (have a real provider_call_id)
-- Leads with stale/empty provider_call_id calls should be re-dialable
AND NOT EXISTS (
