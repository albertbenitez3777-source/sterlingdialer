/*
# Add weekly per-agent performance stats to get_admin_stats
# Daily counts reset automatically (date_trunc('day'))
# Weekly counts cover Mon-Sun of the current week
*/
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'live_humans_week', (SELECT count(*) FROM public.calls WHERE is_live_human = true AND created_at >= v_week_start),
  ...
  FOR v_agent IN SELECT * FROM public.agents WHERE visible_in_admin = true ORDER BY full_name LOOP
    SELECT jsonb_build_object(
      ...
      -- Daily stats (reset automatically each day)
      ...
      'live_humans_week', (SELECT count(*) FROM public.calls WHERE agent_id = v_agent.id AND is_live_human = true AND created_at >= v_week_start),
