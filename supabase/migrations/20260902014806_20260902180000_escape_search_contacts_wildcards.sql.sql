-- v225: Escape SQL LIKE wildcards in search_contacts so _ and % are treated literally.
-- The edge function now pre-escapes search_text with backslash prefixes for %, _, \.
-- The LIKE patterns must use ESCAPE '\' to honor the backslash escaping.

CREATE OR REPLACE FUNCTION public.search_contacts(p_search text DEFAULT ''::text, p_limit integer DEFAULT 50)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  FROM (
    WITH matching_phones AS (
      ...
      WHERE v_digits <> ''
      AND (
        regexp_replace(COALESCE(l.telephone_original, ''), '\D', '', 'g') LIKE '%' || v_digits || '%' ESCAPE '\'
        OR regexp_replace(COALESCE(l.telephone_normalized, ''), '\D', '', 'g') LIKE '%' || v_digits || '%' ESCAPE '\'
      )
      UNION
      SELECT c.consumer_phone AS phone_norm
      FROM public.calls c
      WHERE v_digits <> ''
      AND regexp_replace(COALESCE(c.consumer_phone, ''), '\D', '', 'g') LIKE '%' || v_digits || '%' ESCAPE '\'
    ),
    name_match_phones AS (
      ...
      WHERE v_term_norm <> ''
      AND (
        LOWER(COALESCE(l.name, '')) LIKE '%' || v_term || '%' ESCAPE '\'
        OR regexp_replace(
          regexp_replace(LOWER(COALESCE(l.address, '')), '[,\.]', ' ', 'g'),
          '\s+', ' ', 'g'
        ) LIKE '%' || v_term_norm || '%' ESCAPE '\'
      )
      UNION
      SELECT c.consumer_phone AS phone_norm
      FROM public.calls c
      WHERE v_term <> ''
      AND LOWER(COALESCE(c.consumer_name, '')) LIKE '%' || v_term || '%' ESCAPE '\'
    ),
