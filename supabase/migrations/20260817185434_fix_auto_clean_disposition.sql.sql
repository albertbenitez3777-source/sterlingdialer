/*
# Extend auto-clean timeout and fix disposition for auto-cleaned calls

## Problem
The dialer_next_batch function auto-cleans pending calls after 5 minutes, marking them
is_completed = true with queue = 'pending'. But Bland webhooks sometimes arrive after
5 minutes, so calls get auto-cleaned before the webhook can set the proper disposition
(voice_message, human_drop, fire_transfer). This left 96 calls with no real disposition.

## Changes
1. Extend auto-clean timeout from 5 minutes to 10 minutes — gives webhooks more time.
2. When auto-cleaning, set queue to 'no_answer' instead of leaving as 'pending' —
   this distinguishes auto-cleaned calls from actively-pending calls, and the backfill
   can re-process them when it fetches Bland data.
...
BEGIN
  ...
  -- Auto-clean stale pending calls older than 10 minutes (extended from 5)
  -- Set queue to 'no_answer' so backfill can re-process if webhook arrives later
  UPDATE public.calls
