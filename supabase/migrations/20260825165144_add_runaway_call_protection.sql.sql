-- Runaway call protection: add a safety valve to dialer_next_batch
-- that refuses to create new calls if too many were created recently.
-- Normal rate: ~20 calls/min (concurrency 5, 15s cycles).
-- Safety valve: 60 calls in 2 minutes = 3x normal rate.
-- This catches runaway calling from ANY cause (loop branching, duplicate invocations, etc.)
...
-- RUNAWAY PROTECTION: count calls created in the last 2 minutes
-- If more than 60 calls were created in 120 seconds, refuse to batch.
-- This is 3x the normal rate of ~20 calls/min and catches any runaway scenario.
...
-- Emergency stop the campaign to prevent further damage
...
RETURN jsonb_build_object('success', false, 'error', 'RUNAWAY PROTECTION triggered: ' || v_recent_calls || ' calls created in last 2 minutes. Campaign stopped.');
END IF;
...
AND telephone_normalized !~ '^\+?1?(800|855|866|877|888|844|833)';
