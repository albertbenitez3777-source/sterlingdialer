/*
# Fix search_contacts CTE scope issue

The combined CTE was only visible to the count query but not the results query.
Fix by using a single query that computes both count and paginated results.
Also adds email search capability.
*/
...
CREATE TEMP TABLE _sc_matches ON COMMIT DROP AS
...
lead_data AS (
  SELECT
    'lead' AS source, l.id::text AS id, l.name AS consumer_name,
    l.telephone_original AS phone, l.telephone_normalized AS phone_normalized,
    l.address, l.income_range, l.home_value, l.property_information,
    l.notes, l.original_agent_information,
    l.custom_fields, l.source AS lead_source, l.status AS lead_status,
    l.created_at, l.is_priority,
    NULL::text AS call_queue, NULL::text AS call_disposition,
    NULL::text AS agent_disposition, NULL::text AS agent_notes,
    NULL::timestamptz AS last_call_time, NULL::text AS agent_name,
    NULL::text AS ai_summary, NULL::text AS transcript,
    NULL::text AS recording_url, NULL::integer AS duration_seconds,
    NULL::text AS transfer_status, NULL::boolean AS callback_requested,
    NULL::boolean AS is_dnc, NULL::boolean AS is_wrong_number,
    NULL::boolean AS is_completed, 0 AS total_call_count
  FROM public.leads l
  WHERE phone_last10(l.telephone_normalized) IN (SELECT canon FROM matching_phones)
),
call_data AS (
  SELECT
    'call' AS source, c.id::text AS id, c.consumer_name,
    c.consumer_phone AS phone, c.consumer_phone AS phone_normalized,
    ...
    '' AS notes, NULL::text AS original_agent_information,
    c.consumer_custom_fields AS custom_fields,
    '' AS lead_source, '' AS lead_status,
    c.created_at, false AS is_priority,
    c.queue AS call_queue, c.queue AS call_disposition,
    c.agent_disposition, c.agent_notes,
    c.created_at AS last_call_time, a.full_name AS agent_name,
    c.ai_summary, c.transcript, c.recording_url, c.duration_seconds,
    c.transfer_status, c.callback_requested,
    c.is_dnc, c.is_wrong_number, c.is_completed, 1 AS total_call_count
  FROM public.calls c
...
SELECT DISTINCT ON (phone_last10(phone_normalized))
  ...
  original_agent_information, custom_fields,
...
SELECT count(*) INTO v_total FROM _sc_matches;

SELECT COALESCE(jsonb_agg(row_to_json(c)::jsonb), '[]'::jsonb)
INTO v_results
FROM (SELECT * FROM _sc_matches ORDER BY last_call_time DESC NULLS LAST, created_at DESC LIMIT 50 OFFSET p_offset) c;

DROP TABLE _sc_matches;
