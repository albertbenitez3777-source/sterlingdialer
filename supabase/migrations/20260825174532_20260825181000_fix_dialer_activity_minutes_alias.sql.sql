/*
# Fix get_dialer_activity minutes subquery — use to_jsonb(raw) for aggregate

The minutes subquery alias `raw` needs to be referenced as `to_jsonb(raw)` in the
jsonb_agg call, not `m` or `minute_data`. This is the correct PostgreSQL pattern
for aggregating grouped rows into JSON.
*/
...
BEGIN
  ...
  SELECT jsonb_build_object(
    'minutes', COALESCE((
      SELECT jsonb_agg(to_jsonb(raw) ORDER BY raw.minute DESC)
      FROM (
        SELECT
    ...
    'recent_50', COALESCE((
      ...
      FROM (
        SELECT
          c.id, c.consumer_name, c.consumer_phone, c.queue,
          c.is_live_human, c.is_completed, c.transfer_status, c.transfer_state,
          c.ai_terminated, c.talkroute_answered, c.provider_call_id, c.agent_id,
          a.full_name AS agent_name, c.duration_seconds, c.created_at,
