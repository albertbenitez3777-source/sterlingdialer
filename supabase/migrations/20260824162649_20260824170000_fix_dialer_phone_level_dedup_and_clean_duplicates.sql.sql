/*
# Fix dialer lead waste: phone-level dedup for priority leads, close duplicates, clean stuck leads

1. Changes
- Rewrite dialer_next_batch to add phone-level dedup for PRIORITY leads
  (they previously bypassed dedup entirely, causing re-dials of the same
  phone number when duplicate lead records existed).
- Add phone-level dedup check inside the priority lead loop: before
  inserting a call, check if any existing call for the same phone has
  a real provider_call_id. If so, close the lead and skip.
- Deduplicate leads: close all but one lead record per phone number.
  Keep the oldest lead record as 'new'/'in_progress' and mark the rest
  as 'closed' so they never get dialed.
- Reset stuck in_progress leads back to 'new'.
- No new tables, no new columns, no RLS changes.

2. Rationale
- Last week thousands of dollars were wasted because the same phone
  numbers were dialed multiple times. The root cause was duplicate
  lead records (same phone, multiple rows) combined with priority
  leads bypassing the dedup check that regular leads have.
- This fix adds phone-level dedup to BOTH priority and regular leads,
  and cleans up existing duplicates so the dialer never wastes a call
  on a phone that was already dialed.
*/

-- Step 1: Close all duplicate lead records (keep one per phone number)
-- Keep the oldest record (smallest created_at), close the rest
UPDATE public.leads
SET status = 'closed'
WHERE telephone_normalized <> ''
  AND id NOT IN (
    SELECT id FROM (
      SELECT id, ROW_NUMBER() OVER (PARTITION BY telephone_normalized ORDER BY created_at ASC) as rn
      FROM public.leads
      WHERE telephone_normalized <> ''
    ) t WHERE t.rn = 1
  )
  AND telephone_normalized IN (
    SELECT telephone_normalized
    FROM public.leads
    WHERE telephone_normalized <> ''
    GROUP BY telephone_normalized
    HAVING count(*) > 1
  );

-- Step 2: Reset stuck in_progress leads
UPDATE public.leads SET status = 'new' WHERE status = 'in_progress';

-- Step 3: Rewrite dialer_next_batch with phone-level dedup for ALL leads
...
BEGIN
  ...
  -- Mark leads as 'closed' if their phone has ANY completed call with a real provider_call_id
  UPDATE public.leads
  SET status = 'closed'
  WHERE status = 'in_progress'
  ...
  -- Also close any 'new' leads (priority AND non-priority) that were already called
  UPDATE public.leads
  SET status = 'closed'
  WHERE status = 'new'
    AND telephone_normalized <> ''
  ...
  -- Check if there are priority leads remaining
  ...
  LOOP
    ...
    -- PRIORITY leads first: ONLY status = 'new', with phone-level dedup
    FOR v_lead IN
    ...
    LOOP
      -- Double-check phone hasn't been called (race condition guard)
      SELECT EXISTS(
        SELECT 1 FROM public.calls c
    ...
    -- ONLY pull regular leads if NO priority leads remain
