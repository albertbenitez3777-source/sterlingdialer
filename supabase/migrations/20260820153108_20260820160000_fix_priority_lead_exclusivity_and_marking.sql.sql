/*
# Fix priority lead handling: mark called, call exclusively first

## Problems fixed
1. Priority leads stayed in 'in_progress' or got re-selected because the dialer
   allowed status IN ('new','in_progress','closed'). This caused the same priority
   leads to be re-dialed repeatedly within the same campaign — wasting minutes.
2. Regular leads were being called alongside priority leads because the priority
   loop only filled v_agent_remaining per agent, and if priority leads were
   SKIP LOCKED by another agent, it fell through to the regular lead loop.

## Fix
1. Priority leads are now only selected with status = 'new' (not in_progress or closed).
   After a call is placed, the lead is marked 'in_progress'. The auto-clean logic
   at the top of the function marks them 'closed' once they have a real call record.
2. The regular lead loop is GATED: it only runs if there are NO priority leads
   remaining with status = 'new'. This ensures priority leads are always called
   first and exclusively until exhausted.
3. Added a check at the start: if any priority leads exist with status = 'new',
   skip the regular lead loop entirely.
*/
...
BEGIN
  ...
  -- Priority leads are also marked closed once they have a real call record
  UPDATE public.leads
  SET status = 'closed'
  WHERE status = 'in_progress'
  ...
  WHERE created_at >= v_campaign.started_at
    ...
    AND provider_call_id IS NOT NULL AND c.provider_call_id <> '';
  ...
  LOOP
    ...
    -- PRIORITY leads first: ONLY status = 'new', bypass dedup
    FOR v_lead IN
