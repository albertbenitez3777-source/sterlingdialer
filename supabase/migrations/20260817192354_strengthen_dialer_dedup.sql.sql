/*
# Strengthen duplicate-call prevention in dialer_next_batch

## Problem
The dialer_next_batch function uses NOT EXISTS to skip leads with existing calls,
but between the check and the INSERT, a concurrent loop iteration could insert
the same phone number. The unique index catches it, but the error isn't handled
gracefully — the lead stays "in_progress" forever.

## Fix
1. Use FOR UPDATE SKIP LOCKED on leads to prevent race conditions
2. Add a secondary phone check right before INSERT inside the loop
...
BEGIN
  ...
  LOOP
    ...
    -- Use FOR UPDATE SKIP LOCKED to prevent race conditions between concurrent loop iterations
    FOR v_lead IN
    ...
    LOOP
      -- Double-check: ensure no call was placed to this phone since the query ran
      SELECT EXISTS(
        SELECT 1 FROM public.calls c
      ...
      IF v_phone_already_called THEN
        -- Another agent already called this number — skip it
      ...
      EXCEPTION WHEN unique_violation THEN
        -- Unique constraint violation — another concurrent call already claimed this phone
        UPDATE public.leads SET status = 'closed' WHERE id = v_lead.id;
        -- Don't count this toward remaining
      END;
    END LOOP;
  END LOOP;
