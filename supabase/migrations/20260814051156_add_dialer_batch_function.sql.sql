/*
# Add dialer batch function to pick leads and create call records

The campaign could be "running" but nothing actually dialed leads. This function
is the missing piece: it picks eligible leads, assigns them to the active agent,
creates call records with queue='pending', and marks leads as 'in_progress' so
they won't be dialed twice. It respects the campaign's concurrency and call_limit.

## New Functions
- `dialer_next_batch`: SECURITY DEFINER, admin-only. Returns up to N leads
  ready for dialing, with the active agent's information attached.

## Modified Tables
- `leads`: status updated from 'new' to 'in_progress' for picked leads
- `calls`: new pending call records created for each picked lead
*/
...
BEGIN
  ...
  WHERE active_for_dialer = true AND status = 'active' AND logged_in = true
    AND available_for_transfer = true AND bland_number <> '' AND talkroute_number <> ''
