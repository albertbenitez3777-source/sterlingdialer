...
BEGIN
  ...
  WITH raw(source,id,phone,created_at,rank,profile,custom_fields,submitted,is_dnc,is_wrong_number,is_priority,last_call_time) AS (
    ...
    SELECT r.*, public.phone_last10(r.phone) AS canon,
      ...
      regexp_replace(lower(coalesce(r.profile::text,'')||' '||coalesce(r.custom_fields::text,'')),'[[:space:],.]+',' ','g') AS search_text
    FROM raw r
  ), keys AS (
    SELECT DISTINCT contact_key FROM records
    WHERE (v_phone_search AND strpos(canon,v_digits)>0) OR strpos(search_text,v_norm)>0
  ), matched AS MATERIALIZED (
    SELECT r.* FROM records r JOIN keys k USING(contact_key)
  ), grouped AS (
