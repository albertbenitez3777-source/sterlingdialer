/*
# Fix: dialer_next_batch deletes calls before dialer loop can update them

The DELETE for failed calls was removing ALL calls with empty provider_call_id,
including calls that were just created by the previous batch and are still
being processed by the dialer loop (Bland API calls take several seconds).
This caused a race condition where the self-chaining dialer loop (3s interval)
would trigger dialer_next_batch again, which deleted the just-created calls
before the dialer loop could update them with the provider_call_id.

Fix: Only delete calls with empty provider_call_id that are older than 90 seconds.
This gives the dialer loop enough time to place the Bland call and update the record.
*/
...
BEGIN
  ...
  -- Delete failed call records (never reached Bland) older than 90 seconds
  -- so leads can be retried. The 90-second grace period gives the dialer loop
  -- enough time to place the Bland call and update provider_call_id.
