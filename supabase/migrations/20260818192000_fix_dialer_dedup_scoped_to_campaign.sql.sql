/*
# Fix dialer dedup to scope by campaign started_at

## Problem
The dialer_next_batch function skipped any lead whose phone number had
ANY outbound call record with a real provider_call_id — even from previous
campaigns. This meant that when a new campaign started, all previously-called
leads were permanently blocked, even though the campaign was fresh.

## Fix
Changed the dedup logic in dialer_next_batch to only skip phone numbers
that have a call record with a real provider_call_id created AFTER the
current campaign's started_at. This way:
- Leads called in a previous campaign can be retried in a new campaign
- Leads called in the current campaign are still skipped (no double-dialing)
- The unique constraint on outbound calls is scoped to the current campaign

## Modified Function
- `dialer_next_batch()` — dedup queries now use `c.created_at >= v_campaign.started_at`
*/
...
BEGIN
  ...
  -- SINCE THIS CAMPAIGN STARTED (not from all history)
  UPDATE public.leads
  SET status = 'closed'
  WHERE status = 'in_progress'
