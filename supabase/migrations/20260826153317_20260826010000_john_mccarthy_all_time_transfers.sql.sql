/*
# Show all-time transfers for John McCarthy only

## Context
The get_agent_queues function filters calls to the current week (v_week_start).
John McCarthy (1b01a583-98c5-439a-999e-d312c68106c3) has 387 all-time fire transfers
but only sees 58 this week. The user wants ALL his transfers visible from day 1.

## Change
- For John McCarthy's agent_id, skip the week_start filter on fire_transfers and human_drop
- All other agents keep the current week filter unchanged
- Voice messages queue unchanged for all agents
*/
...
-- John McCarthy sees all-time transfers; everyone else sees current week only
IF p_agent_id = '1b01a583-98c5-439a-999e-d312c68106c3'::uuid THEN
  v_cutoff := '2020-01-01'::timestamptz;
ELSE
  v_cutoff := v_week_start;
END IF;
...
-- Voice messages stay week-filtered for all agents
SELECT COALESCE(jsonb_agg(row_to_json(r) ORDER BY r.created_at DESC), '[]'::jsonb)
