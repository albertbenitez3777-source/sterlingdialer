/*
# Include all agents (including archived/inactive) in admin stats

## Changes
- Updates get_admin_stats to loop over ALL agents, not just active ones
- Archived agents (John McCarthy, Matt Vargas, William Styles, Dave Sayer) will now appear in the roster
- Their status field distinguishes them from active agents
- No changes to campaign, calls, or dialer logic

## Why
- Owner/supervisor need to see full roster including OFF/archived agents
- Required for complete operational visibility
*/
...
'transfer_failed_unverified_today', (SELECT count(*) FROM public.calls WHERE transfer_status IN ('failed','transfer_failed_unverified') AND created_at >= date_trunc('day', now())),
...
'daily_minutes_used', COALESCE((SELECT sum(duration_seconds)::numeric / 60 FROM public.calls WHERE call_direction = 'outbound' AND created_at >= date_trunc('day', now())), 0),
...
SELECT jsonb_build_object(
'callbacks_today', (SELECT count(*) FROM public.calls WHERE agent_id = v_agent.id AND callback_requested = true AND created_at >= date_trunc('day', now())),
...
'productive_minutes_today', COALESCE((SELECT sum(duration_seconds)::numeric / 60 FROM public.calls WHERE agent_id = v_agent.id AND duration_seconds >= 30 AND created_at >= date_trunc('day', now())), 0),
'wasted_minutes_today', COALESCE((SELECT sum(duration_seconds)::numeric / 60 FROM public.calls WHERE agent_id = v_agent.id AND duration_seconds < 30 AND created_at >= date_trunc('day', now())), 0),
'total_minutes_today', COALESCE((SELECT sum(duration_seconds)::numeric / 60 FROM public.calls WHERE agent_id = v_agent.id AND created_at >= date_trunc('day', now())), 0)
) INTO v_stats2;
