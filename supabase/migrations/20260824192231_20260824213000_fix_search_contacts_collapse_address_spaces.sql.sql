/*
# Fix search_contacts — collapse spaces in address normalization

## Problem
After removing commas from addresses like "Miami, FL 33180", the result was
"miami  fl 33180" (two spaces where the comma was). The search term "miami fl"
(one space) didn't match because LIKE '%miami fl%' doesn't match "miami  fl".

## Fix
Apply regexp_replace to collapse multiple spaces into one on BOTH the search term
AND the address before comparing. Also collapse spaces on the address side in the
LIKE comparison.
