/*
# Fix Transfer Protection and Add Redial Cap

## Problem
The dialer_next_batch() auto-clean was killing calls at 45 seconds — mid-transfer.
Cold transfers to Talkroute take 60-120 seconds to ring, answer, and bridge.
This was causing transfer failures and wasting minutes on redials.

## Changes
1. Raise auto-clean threshold from 45 seconds to 150 seconds.
2. Add exclusions to the auto-clean WHERE clause so it NEVER touches calls
   that have begun transferring (queue = 'fire_transfer', transfer_requested_at
   IS NOT NULL, talkroute_leg_created = true, ai_terminated = true).
3. Add redial_count column to calls table to track re-dials per contact.
4. Add index on redial_count for fast filtering.
...
-- Add redial_count column to calls table
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'calls' AND column_name = 'redial_count'
  ) THEN
    ALTER TABLE public.calls ADD COLUMN redial_count integer NOT NULL DEFAULT 0;
  END IF;
END $$;

-- Add index for redial_count lookups
CREATE INDEX IF NOT EXISTS idx_calls_redial_count ON public.calls (redial_count);

-- Recreate dialer_next_batch with 150-second threshold and transfer exclusions
...
BEGIN
  ...
  -- Auto-clean stale pending calls (raised to 150s; excludes mid-transfer calls)
  UPDATE public.calls
  ...
  WHERE queue = 'pending' AND is_completed = false AND created_at < now() - interval '150 seconds'
  ...
  -- Reset in_progress leads with no active call
