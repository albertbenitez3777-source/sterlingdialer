-- Fix dialer_next_batch: skip leads by PHONE NUMBER, not just lead_id.
-- The same phone number exists across multiple leads (duplicates from CSV imports).
-- Previously the dialer only checked if a lead_id had a call with a provider_call_id,
-- so the same person got called once per duplicate lead row.
-- Now we skip ANY lead whose phone number has already been sent to Bland.
-- Also fix the auto-clean to set stale pending calls to 'pending' (not 'human_drop')
-- so they don't flood the agent inbox.
...
BEGIN
  ...
  -- Auto-clean stale pending calls older than 10 minutes (mark completed, keep as pending queue)
  UPDATE public.calls
  ...
  LOOP
    ...
    FOR v_lead IN
      ...
      -- Skip ANY lead whose phone number has already been called with a real provider_call_id.
      -- This prevents the same person from being called multiple times across duplicate lead rows.
      AND NOT EXISTS (
