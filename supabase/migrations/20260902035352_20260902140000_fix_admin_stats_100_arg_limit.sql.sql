-- Fix: jsonb_build_object has a 100-argument limit. Split the per-agent stats
-- into two jsonb_build_object calls and merge with ||.
...
-- Part 2: all-time + advanced metrics (split to avoid 100-arg limit)
SELECT jsonb_build_object(
