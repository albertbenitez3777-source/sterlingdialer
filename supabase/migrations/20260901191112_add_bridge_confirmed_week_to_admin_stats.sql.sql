/*
# Add weekly bridge_confirmed and talkroute_answered to admin stats

1. Changes
- Adds `bridge_confirmed_week` and `talkroute_answered_week` to the per-agent stats
  in `get_admin_stats()`, scoped to the current week (Monday–Sunday ET).
- These mirror the existing `bridge_confirmed_today` / `talkroute_answered_today`
  fields but use the weekly date boundary so the Agent Performance "THIS WEEK"
  tab can show successful Talkroute handoffs.
2. Security
- No table or policy changes. The function is already SECURITY DEFINER.
*/
...
BEGIN
  SELECT jsonb_build_object(
    'campaign_state', COALESCE((SELECT state FROM public.campaign LIMIT 1), 'idle'),
    'dialer_activated', COALESCE((SELECT dialer_activated FROM public.campaign LIMIT 1), false),
    'concurrency', COALESCE((SELECT concurrency FROM public.campaign LIMIT 1), 1),
    'provider_call_limit', COALESCE((SELECT provider_call_limit FROM public.campaign LIMIT 1), 100),
    ...
    'blocking_reason', COALESCE((SELECT blocking_reason FROM public.campaign LIMIT 1), ''),
    'campaign_started_at', (SELECT campaign_started_at FROM public.campaign LIMIT 1),
    ...
    'dialer_status', COALESCE((SELECT dialer_status FROM public.campaign LIMIT 1), 'idle'),
