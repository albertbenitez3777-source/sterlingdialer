-- Block toll-free numbers (800, 855, 866, 877, 888, 844, 833) from being dialed.
-- These are company/business numbers, not residential — calling them wastes minutes.
-- Also close any existing toll-free leads so they're never picked up.

-- Close any toll-free leads that are currently new or in_progress
UPDATE public.leads
SET status = 'closed'
WHERE status IN ('new', 'in_progress')
  AND telephone_normalized ~ '^\+?1?(800|855|866|877|888|844|833)';

-- Recreate dialer_next_batch with toll-free exclusion added to both priority and non-priority lead queries
