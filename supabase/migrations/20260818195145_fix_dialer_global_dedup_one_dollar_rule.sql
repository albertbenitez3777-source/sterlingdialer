/*
# Fix dialer dedup: "One Dollar Only" rule — global dedup, not per-campaign

## Problem
The dialer_next_batch function only skipped phone numbers that had a call
record with a real provider_call_id created AFTER the current campaign's
started_at. This meant that every time a campaign restarted, ALL previously-
called leads became eligible again — resulting in the same leads being
called 3-4 times across campaign restarts, wasting Bland minutes.

## Fix
Changed ALL dedup queries in dialer_next_batch to check against the ENTIRE
call history (removed the `c.created_at >= v_campaign.started_at` filter).
This enforces the "one dollar only" rule: each phone number is dialed exactly
ONE time, period — regardless of how many campaigns have been started.

## Also fixed
- The "mark closed" query now checks ALL call history, so leads that were
  called in a previous campaign stay permanently closed.
- The "reset in_progress to new" query already only resets leads with NO
  real call records, so that's correct — but now the "mark closed" query
  will properly close them if they have ANY real call record from any time.
*/
...
BEGIN
  ...
  -- Mark leads as 'closed' if their phone has ANY completed call with a real
  -- provider_call_id — checking ALL call history, not just current campaign.
  -- This enforces the "one dollar only" rule: each number is dialed once, ever.
  UPDATE public.leads
  SET status = 'closed'
  WHERE status = 'in_progress'
  ...
  -- Also close any 'new' leads that were already called in a previous campaign
  -- but somehow still have status = 'new' (e.g. campaign_start didn't close them)
  UPDATE public.leads
  SET status = 'closed'
  WHERE status = 'new'
    AND telephone_normalized <> ''
  ...
  -- Count only successfully placed calls SINCE THIS CAMPAIGN STARTED
  -- (this is for the call limit, not for dedup)
