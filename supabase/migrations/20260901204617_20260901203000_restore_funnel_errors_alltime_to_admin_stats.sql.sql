-- Restore funnel_today/week/all, errors_recent, all-time stats, and
-- summary-level transfer fields to get_admin_stats().
-- These were lost when later migrations overwrote the function without
-- preserving the observability additions from migration 20260901180917.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'fire_transfers_all', (SELECT count(*) FROM public.calls WHERE queue = 'fire_transfer'),
    'human_drops_all', (SELECT count(*) FROM public.calls WHERE queue = 'human_drop'),
    'live_humans_all', (SELECT count(*) FROM public.calls WHERE is_live_human = true),
    ...
    -- Summary-level transfer funnel fields
    ...
    'transfers_requested_week', (SELECT count(*) FROM public.calls WHERE (queue = 'fire_transfer' OR transfer_requested_at IS NOT NULL) AND created_at >= date_trunc('week', now())),
    ...
    'transfer_failed_unverified_today', (SELECT count(*) FROM public.calls WHERE queue = 'human_drop' AND transfer_failure_reason <> '' AND created_at >= date_trunc('day', now())),
    'transfer_failed_unverified_week', (SELECT count(*) FROM public.calls WHERE queue = 'human_drop' AND transfer_failure_reason <> '' AND created_at >= date_trunc('week', now())),
