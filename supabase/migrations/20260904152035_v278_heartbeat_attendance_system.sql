/*
# Heartbeat-based attendance system

## Purpose
Replace unreliable agents.logged_in flag with server-stamped heartbeat presence.

## Modified Tables
- auth_sessions: ADD last_heartbeat_at (timestamptz, nullable)

## New Functions
- heartbeat_session(p_session_token text) — SECURITY DEFINER
  Validates session, stamps last_heartbeat_at = now(), returns attendance summary
  (presence state, today/week totals, current session duration).
  Uses verified PIN session as authority.
...
- EXECUTE restricted to authenticated only (no anon, no public)
- No raw session tokens in attendance output
- No new RLS policies needed

## Important Notes
1. Historical sessions without last_heartbeat_at use expires_at/invalidated_at
   as interval endpoint — labeled "legacy estimate" in UI.
2. 90-second stale threshold: no heartbeat for 90s = disconnected.
3. agents.logged_in kept for dialer backward compatibility.
4. Business timezone: America/New_York, Monday-based weeks.
*/
