-- Increase the failed-call delete grace period from 90s to 3 minutes
-- The dialer loop takes ~24 seconds to place 12 calls (1s delay between each),
-- plus 3s self-chain delay. 90s was too short when multiple batches overlap.
UPDATE public.calls SET is_completed = true, queue = 'no_answer'
WHERE queue = 'pending' AND is_completed = false AND created_at < now() - interval '10 minutes';

-- Just re-create the function with the 3-minute threshold
