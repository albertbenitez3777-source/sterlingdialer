/*
# Add all-time transfer/human counts to admin stats

## Context
The admin dashboard "Re-Dial Live Transfers & Live Humans" section shows per-agent
counts of fire_transfers and live_humans. These were previously week-only counts.
The user wants all-time counts from day 1 displayed there.

## Changes
- Adds `fire_transfers_all`, `human_drops_all`, and `live_humans_all` fields to
  each agent row in get_admin_stats().
- Adds the same all-time fields to the campaign summary.
- Today and week fields remain unchanged for the performance table.
*/
...
'live_humans_all', (SELECT count(*) FROM public.calls WHERE queue IN ('fire_transfer','human_drop'))
) INTO v_summary;
...
'live_humans_all', (SELECT count(*) FROM public.calls WHERE agent_id = v_agent.id AND queue IN ('fire_transfer','human_drop'))
) INTO v_stats;
