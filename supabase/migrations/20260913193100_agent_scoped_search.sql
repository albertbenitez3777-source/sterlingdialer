CREATE OR REPLACE FUNCTION public.search_contacts_scoped(p_search text, p_limit integer, p_offset integer, p_agent_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  WITH matching(source,id,canon) AS MATERIALIZED (
    SELECT 'lead',l.id::text,public.phone_last10(coalesce(nullif(l.telephone_normalized,''),l.telephone_original)) FROM (SELECT * FROM public.leads WHERE assigned_agent_id = p_agent_id) l
    ...
    UNION ALL
    SELECT 'call',c.id::text,public.phone_last10(coalesce(nullif(c.consumer_phone,''),CASE WHEN c.call_direction='inbound' THEN c.from_number ELSE c.to_number END)) FROM (SELECT * FROM public.calls WHERE agent_id = p_agent_id) c
    ...
    UNION ALL
    SELECT 'retry',r.id::text,public.phone_last10(r.phone_normalized) FROM (SELECT * FROM public.retry_leads WHERE previous_agent_id = p_agent_id) r
    ...
    UNION ALL
    SELECT 'transfer',t.id::text,public.phone_last10(t.phone_normalized) FROM (SELECT * FROM public.transfer_context WHERE agent_id = p_agent_id) t
    ...
    UNION ALL
    SELECT 'secretary',s.id::text,public.phone_last10(s.client_phone) FROM (SELECT * FROM public.secretary_calls WHERE agent_id = p_agent_id) s
    ...
    UNION ALL
    SELECT 'saved_transfer',s.id::text,public.phone_last10(s.consumer_phone) FROM (SELECT * FROM public.saved_transfers WHERE agent_id = p_agent_id) s
    ...
    FROM (SELECT * FROM public.leads WHERE assigned_agent_id = p_agent_id) l
    UNION ALL
    ...
    FROM (SELECT * FROM public.calls WHERE agent_id = p_agent_id) c
    UNION ALL
    ...
    FROM (SELECT * FROM public.retry_leads WHERE previous_agent_id = p_agent_id) r
    UNION ALL
    ...
    FROM (SELECT * FROM public.transfer_context WHERE agent_id = p_agent_id) t
    UNION ALL
    ...
    FROM (SELECT * FROM public.secretary_calls WHERE agent_id = p_agent_id) s
    UNION ALL
    ...
    FROM (SELECT * FROM public.saved_transfers WHERE agent_id = p_agent_id) s
...
REVOKE ALL ON FUNCTION public.search_contacts_scoped(text,integer,integer,uuid) FROM public,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.search_contacts_scoped(text,integer,integer,uuid) TO service_role;
