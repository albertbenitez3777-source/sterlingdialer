...
export function WhatsUp({sessionToken,agentId,onUnauthorized}:{sessionToken:string;agentId:string;onUnauthorized:()=>void}) {
 ...
 return <aside className={`whatsup ${open?'expanded':''}`} aria-label="WhatsUp team communication">
  ...
  <div className="whatsup-call"><button disabled={observer||!!video} onClick={()=>setVideo({room,title})}><Video size={15}/> Live call</button><span>Team chat is live. Video needs LIVEKIT_URL + API key/secret on the server. If Join is blocked, the owner has not connected video yet.</span></div></>}
